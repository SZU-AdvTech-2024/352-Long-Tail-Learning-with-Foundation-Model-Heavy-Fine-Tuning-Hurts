...
class Trainer:
    ...
    def build_data_loader(self):
        ...
        if cfg.dataset in ["CIFAR100", "CIFAR100_IR10", "CIFAR100_IR50", "CIFAR100_IR10_NR50"]:
    ...
    def build_optimizer(self):
        ...
        self.optim = torch.optim.SGD([{"params": self.tuner.parameters()},
                                      {"params": self.head.parameters()}, {"params": self.model.temperature},
                                      {"params": self.model.scale}],
    ...
    def train(self):
        ...
        for epoch_idx in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                sum = len(label)
                with torch.no_grad():
                    # probs = model2(image).softmax(dim=-1).to(self.device)
                    output2 = model2(image).to(self.device)
                    output3 = model2(image2).to(self.device)
                    probs = output3.softmax(dim=-1)
                    for i in range(len(probs)):
                    #     refinement = torch.zeros(100)
                        values, indices = probs[i].topk(1)
                        if label[i] == indices[0]:
                            true = true + 1
                ...
                if cfg.prec == "amp":
                    with (autocast()):
                        ...
                        loss2 = F.kl_div(F.log_softmax(output/self.model.temperature, dim=1), F.softmax(output2/self.model.temperature, dim=1), reduction='batchmean') * self.model.scale
                        # default: 53
                        if true/sum < 0.53:
                            ajust_num = ajust_num + 1
                            loss_micro = (samples * loss + (1 - samples) * loss2) / self.accum_step
                        else:
            ...
            print("ajust numbers:{}".format(ajust_num))
            self.sched.step()
            torch.cuda.empty_cache()
